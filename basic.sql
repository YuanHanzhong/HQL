...
select
    *
  , count( ) over ( partition by name order by orderdate rows between unbounded preceding and unbounded following )
from business b;
...
from (
         select
             name
           , count( ) over (partition by month( orderdate ) rows between unbounded preceding and unbounded following) as cn
         from business b
         where
             month( orderdate ) = 4
     ) t1
...
select
  ...
  , orderdate
  , cost
  , count( * ) over (rows between unbounded preceding and current row) as cn
from business;
...
from business b
where
    month( orderdate ) = 4 and year( orderdate ) = 2022
...
select *
from (
         select name, month( orderdate ) as ordermonth, cost
         from business b
     ) t1
where
    ordermonth = 4
...
-- 这个开窗运用的很巧妙
select *, sum( cost ) over (partition by name order by orderdate rows between 1 preceding and current row) as last_two
from business b;
...
select
  ...
  , coalesce( cost, 0 ) + coalesce( preceed_cost, 0 )
    ...
    -- lag
from (
         select *, lag( cost, 1, 0 ) over (partition by name order by orderdate) as preceed_cost
         from business b
     ) t1;
...
from (
         select *, lag( cost ) over (partition by name order by orderdate) as preceed_cost
         from business b
     ) t1;
...
-- ntile 必须要和 order by 一起使用, 有了顺序才能划分组
...
select
  ...
  , first_value( orderdate )
                 over (partition by name,month( orderdate ) order by orderdate rows between UNBOUNDED PRECEDING and UNBOUNDED following) as first_value
  , first_value( orderdate )
                 over (partition by name,month( orderdate ) order by orderdate rows between UNBOUNDED PRECEDING and UNBOUNDED following) as first_value
  , last_value( orderdate )
                over (partition by name,month( orderdate ) order by orderdate )                                                          as last_value1
  , last_value( orderdate )
...
from (
         ...
         from (
                  select name, month( orderdate ) as order_month, cost
                  from business b
              ) t1
         where
             order_month = 4
...
from (
         select name
         from (
                  select name, month( orderdate ) as date_month
                  from business b
              ) t1
         where
             date_month = 4
...
from (
         ...
         from business b
         where
             month( orderdate ) = 4 -- where后的条件不一定要出现在select后面
     ) t1
group by name
;
...
select
  ...
  , first_value( orderdate )
                 over (partition by name,month( orderdate ) order by orderdate rows between UNBOUNDED PRECEDING and UNBOUNDED following) as first_value
  , last_value( orderdate )
                over (partition by name,month( orderdate ) order by orderdate rows between UNBOUNDED PRECEDING and UNBOUNDED following)  as last_value
from business
-- 14）查询前20%时间的订单信息
select *
from (
         select *, ntile( 5 ) over (partition by name order by orderdate) as ntile_5
         from ( business b )
     ) t1
where
    ntile_5 = 1
...
from (
         select *, sum( cost ) over (partition by name) as sum_cost
         from ( business b )
     ) t1
...
select
  ...
  , orderdate
  , cost
  , rank( ) over (partition by name order by cost desc)       as rk
  , dense_rank( ) over (partition by name order by cost desc) as drk
  , row_number( ) over (partition by name order by cost desc) as drk
from business
;
show functions;
...
select distinct
    month( hiredate_fromatted )                                                                     as month_num
  , count( * ) over (partition by month( hiredate_fromatted ) order by month( hiredate_fromatted )) as cn_month
from (
         select name, hiredate, replace( hiredate, '/', '-' ) as hiredate_fromatted
         from employee e
     ) e
...
from (
         select
             *
           , month( replace_hiredate ) as month_hiredate
         from (
                  select *, replace( hiredate, '/', '-' ) as replace_hiredate
                  from ( employee e )
              ) t1
     ) t2
...
select
  ...
  , year( replace( birthday, '/', '-' ) )  as birth_year
  , month( replace( birthday, '/', '-' ) ) as birth_month
from employee e;
...
select
    name
  , birthday
  , `if`( current_month < birth_month, current_year - birth_year - 1, current_year - birth_year )      as `years_old`
  , `if`( current_month < birth_month, 12 + current_month - birth_month, current_month - birth_month ) as `month_old`
from (
         -- 最基础的数据拿出来, 放到一起  非常标准, 非常棒
         select
             name
           , birthday
           , year( `current_date`( ) )              as current_year
...
select
    name
  , `if`( month_current_date > month_birthday, year_current_date - year_birthday,
          year_current_date - year_birthday - 1
        )                               as age_year
  , month_current_date - month_birthday as age_month
from (
         -- 预处理数据, 减少重复计算, 看起来, 写起来也更容易.
         -- datediff 只能接受日期作为参数, 对格式要求很严格. 并且结果只能以天为单位
         select
             name
           , year( birthday )           as year_birthday
           , month( birthday )          as month_birthday
           , year( `current_date`( ) )  as year_current_date
           , month( `current_date`( ) ) as month_current_date
         from (
                  select name, replace( birthday, '/', '-' ) as birthday
                  from employee
              ) t1
     ) t2
...
from (
         select
           ...
           , `if`(
                     month( `current_date`( ) ) > month( birthday_formatted )
                 , month( `current_date`( ) ) - month( birthday_formatted )
                 , month( `current_date`( ) ) - month( birthday_formatted ) + 12 ) as monthes
         from (
                  select name, ( replace( birthday, '/', '-' ) ) as birthday_formatted
                  from employee e
              ) t1
     ) t12
...
select
  ...
  , abs_month
  , r_birthday
  , concat( age_year, "岁", age_month, "月" ) as age
from (
         select
             name
           , abs_year
           , abs_month
           , r_birthday
           , `if`( abs_month < 0, abs_year - 1, abs_year )    as age_year --可以在这里concat，但只能用很长的式子，是就会显得乱
           , `if`( abs_month < 0, 12 + abs_month, abs_month ) as age_month
         from (
                  select
                      name
                    , r_birthday
                    , year( `current_date`( ) ) - year( r_birthday )   as abs_year
                    , month( `current_date`( ) ) - month( r_birthday ) as abs_month
                  from (
                           select *, replace( birthday, '/', '-' ) as r_birthday
                           from ( employee e )
                       ) t1
              ) t12
     ) t3
...
select
    name
  , r_birthday
  , datediff( `current_date`( ), r_birthday ) as days
from (
         select *, replace( birthday, '/', '-' ) as r_birthday
         from ( employee e )
     ) t1
...
select
    name
  , children
  , map_keys( children ) as children_name
  , map_values( children )
...
select job, sum( `男` ) as `男人数`, sum( `女` ) as `女人数`
from (
         -- 第一步就是把基本数据摘出来
         select name, sex, job, if( sex = '男', 1, 0 ) as `男`, if( sex = '女', 1, 0 ) as `女`
         from employee e
     ) t1
group by job;
...
--      if 判断
--      都填充 1, 直接 select 1
select first_name, sum( one ) as sum_one, collect_set( stu_name )
from (
         select stu_name, substr( stu_name, 1, 1 ) as first_name, 1 as one
         from student s
     ) t1
group by first_name
...
select `First Name`
from (
         select *, substr( stu_name, 1, 1 ) as `First Name`
         from student
     ) t1
group by `First Name`
having
...
select
  ...
  , count( * )
from (
         select *, substr( stu_name, 0, 1 ) as first_name
         from ( student s )
     ) t1
group by first_name
having
...
select
    stu_id
  , sum( `if`( course_name = '语文', course, 0 ) ) as `语文`
    --  sum if 就是在一列变多列的时候使用
    -- partition by不会去重, 只是多了一列
from (
         select stu_id, course_name, course
         from course     c
              join score s on s.course_id = c.course_id
     ) t1
...
select
    stu_id
  , sum( `if`( course_name = "语文", course, 0 ) ) as `语文`
  , sum( `if`( course_name = "数学", course, 0 ) ) as `数学`
  , sum( `if`( course_name = "数学", course, 0 ) ) as `数学`
  , sum( `if`( course_name = "体育", course, 0 ) ) as `体育`
  , sum( `if`( course_name = "音乐", course, 0 ) ) as `音乐`
  , count( * )                                     as `有效课程数`
  , sum( course )                                  as `总成绩`
  , sum( course ) / count( * )                     as `平均成绩`
  , count( c.course_id )                           as `有效科目数量`
  , count( * )                                     as `有效科目数量`
    -- count(*) 会统计所有行, 包括空行,
...
where
        ...
        (
            ...
            from (
                     select *, sum( bigger_60 ) over (partition by stu_id) as sum_bigger_60
                     from (
                              select stu_id, course_id, course, `if`( course > 60, 1, 0 ) as bigger_60
                              from score s
                          ) t1
                 ) t2
            where
...
from student s
         -- 要什么给什么即可, 没有必要全部都弄出来
     join (
              -- 开窗, 设置标志位, 筛选是非常好的方式. 设置标志位非常巧妙
              
              select stu_id, sum( if( course < 60, 0, 1 ) ) as sum_under_60
              from score s
              group by stu_id
              having
                  sum_under_60 < 1
...
-- 2023年08月29日
select stu_id, count( * ) as cn, avg( course )
from score s
where
    course < 60
group by stu_id
...
where
        ...
        (
            select distinct stu_id
            from (
                     select stu_id, flag, max( flag ) over () as max_flag
                     from (
                              -- 1. 原始数据
                              select c.course_id, stu_id, count( ) over (partition by stu_id) as flag
                              from teacher t left join course c on c.tea_id = t.tea_id
...
from (
         ...
         --  细细体味, 在没有重复数据的情况下, row_number和 count 的不同
         select c.course_id, stu_id, row_number( ) over (partition by stu_id) as flag
         from teacher t left join course c on c.tea_id = t.tea_id
                        left join score s on s.course_id = c.course_id
         where
             tea_name = '李体音'
...
select
from (
         select stu_id, max( flag ) as max_flag
         from (
                  -- 1. 原始数据
                  select c.course_id, stu_id, count( ) over (partition by stu_id) as flag
                  from teacher t left join course c on c.tea_id = t.tea_id
...
select
    stu_id
  , array_contains( c_course_id )
from (
         select stu_id, collect_set( course_id ) as c_course_id
         from score s
         group by stu_id
     ) t1
