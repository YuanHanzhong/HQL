...
select
    stu_name
  , year( `current_date`( ) ) - year( birthday )   as year
  , month( `current_date`( ) ) - month( birthday ) as month
from student;
...
from (
         ...
         from (
                  select
                      id
                    , DENSE_RANK( ) over (order by `sum` desc) as rk
                  from order_summary
              ) t1
...
from (
         select
           ...
           , date_sub( active_date, row_number( ) over (partition by user_id order by active_date) ) as flag
         from (
                  select
                      user_id
                    , active_date
...
having
    count( flag ) >= 3 -- 连续登录大于等于三天
;


/*
2.2.1 查询各学生的年龄（精确到月份)
...
select
  ...
  , stu_name
  , birthday
  , if( month( current_timestamp ) - month( birthday ) < 0, year( current_timestamp ) - year( birthday ) - 1,
...
-- 2022/10/21 7:58  日期先减再转化会报错, 要想转化, 再相减
...
from (
         select
             stu_id
           , birthday
           , year( `current_date`( ) ) - year( birthday )   as `sui`
           , month( `current_date`( ) ) - month( birthday ) as `yue`
         from student
     ) t;
...
select
  ...
  , `if`( month( `current_date`( ) ) - month( birthday ) >= 0, year( `current_date`( ) ) - year( birthday ),
          year( `current_date`( ) ) - year( birthday ) - 1 )    as age_year
  , `if`( month( `current_date`( ) ) - month( birthday ) >= 0, month( `current_date`( ) ) - month( birthday ),
          month( `current_date`( ) ) - month( birthday ) + 12 ) as age_month


from student s;

-- 2022/10/19 11:07  concat比较复杂的时候, 就弄个中间表

-- 2022/10/19 10:08  取小数
...
from (
         ...
         from score s
         group by stu_id
                  -- 2022/10/19 14:02  至少参加1门考试
         having
             count( * ) > 1
     ) t;


-- 2022/10/8 11:44  count时, 注意1非空, 2重复
select count( distinct stu_id ) as `参加考试的学生个数`
from score
where
    course is not null;
...
select course_id, min( course ) as min, max( course ) as max
from score
group by course_id
;
...
select stu_id, avg( course ) as avg
from score
group by stu_id
having
        avg
...
select *
from (
         select stu_id, avg( course ) as avg_score
         from score
         group by stu_id
     ) t1
where
...
select stu_id, count( distinct course_id ) as num
from score
group by stu_id
having
    num >= 2
...
select firstname, count( * ) as `人数`
from (
         select substr( stu_name, 1, 1 ) as firstname
         from student
     ) t1
...
select course_id, avg( course ) as avg
from score
group by course_id
order by avg( course )
;
...
-- 3.4.1 查询学生的总成绩并进行排名
select stu_id, sum( course ) as sum
from score
group by stu_id
order by sum desc
;
...
select
    stu_id
  , sum( if( course_name = "语文", course, 0 ) ) as `yuwen`
  , sum( if( course_name = "数学", course, 0 ) ) as `shuxue`
  , sum( if( course_name = "英语", course, 0 ) ) as `yingyu`
  , sum( if( course_name = "体育", course, 0 ) ) as `tiyu`
  , sum( if( course_name = "音乐", course, 0 ) ) as `yinyue`
  , avg( course )                                as `avg_score`
  , sum( if( course = 0, 1, 1 ) )                as `有效课程数`
...
select
  ...
  , sum( if( c.course_name = '语文', course, 0 ) ) as `语文`
from student     s
     join score  s2
     on s2.stu_id = s.stu_id
     join course c
     on c.course_id = s2.course_id
...
from (
         (
             select t.stu_id, stu_name, course as `数学`
             from (
                      select *
                      from student     s
                           join score  s2
             ...
             join
             (
                 select t.stu_id, stu_name, course as `语文`
                 from (
                          select *
                          from student     s
                               join score  s2
...
select
  ...
  , (
        ...
        from score
             join course c
             on c.course_id = score.course_id
        where
              score.stu_id = t.stu_id
          and course_name = '语文'
    )                             as `语文`
...
where
        ...
        (
            ...
            from score
                 join course c2
                 on score.course_id = c2.course_id
            where
                course_name = '语文'
            -- 2022/10/9 12:09  嵌套子查询报错不支持的时候, 考虑先join, 需要哪些数据, 哪些表, 先join下
            --                          (
            --                              select course_id
            --                              from course
...
select stu_id, avg( course ) as `不及格课程的平均成绩`, count( course ) as `不及格课程数`
from score s
where
    course < 60
group by stu_id
...
select stu_id, avg( course ), count( course ) as `不及格课程数`
from score
where
        stu_id in
        (
            -- 2022/10/9 14:46 只有下面这个查询的时候, 这个只是算的不及格的同学的平均成绩  这个是错的
            select
                stu_id
                --                ,avg(course)
            from score
...
select s.stu_id as `学号`, stu_name as `姓名`, count( course ) as `选课数`, sum( course ) as `总成绩`
from student
     join score s
     on student.stu_id = s.stu_id
...
from (
         select
             stu.stu_id
           , stu.stu_name
           , avg( course ) over (partition by stu.stu_id) as ag
         from score             sc
...
select course_id, sum( if( course >= 60, 1, 0 ) ) as bigger_60, sum( `if`( course < 60, 1, 0 ) ) as less_60
from score s
group by course_id;
...
from (
         select course_id, count( * ) as `不及格人数`
         from score s
         where
             course < 60
         group by course_id
     ...
     join
          (
              select course_id, count( * ) as `及格人数`
              from score s
              where
                  course > 60
              group by course_id
     ...
     join course
     on t1.course_id = course.course_id
...
select
  ...
  , sum( `if`( course <= 85 and course > 70, 1, 0 ) )  as `(70-85]`
  , sum( `if`( course <= 70 and course > 60, 1, 0 ) )  as `(60-70]`
  , sum( `if`( course <= 60, 1, 0 ) )                  as `<60`
from score s
group by course_id
;
...
-- 2022/11/2 19:00  用flag还是low了, 用exists 最高级
...
select
    stu_id
  , min( if( course_id = '01', course, 0 ) ) as `01`
  , min( if( course_id = '02', course, 0 ) ) as `02`
  , min( if( course_id = '03', course, 0 ) ) as `03`
  , max( if( course_id = '04', course, 0 ) ) as `04`
from score
group by stu_id;
...
where
            course_id = '01'
  and       course < 60 distribute by course
    sort by course desc
;
...
from (
         ...
         from (
                  ...
                  where
                      course < 60
                  group by s.stu_id, stu_name, course
              ) t1
...
from score      s
     join score s2
     on s.stu_id <> s2.stu_id and s.course = s2.course
...
from (
         select
             s1.stu_id
           , s1.stu_name
           , s1.course as 01course
           , s2.course as 02course
         from (
                  select
                      s.stu_id
                    , stu_name
...
-- 想要用统计的方法筛选出来, 应该放到 1 列
--
select stu_id
from (
         select
             stu_id
           , sum( `if`( course_id = '01' or course_id = '02', 1, 0 ) ) as sel_01_02
             --              , sum(`if`(course_id='02',1,0)) sel_02
...
from (
         select
             *
           , `if`( course_id = '01', 1, 0 ) as sel_01_02
         from score
         union all
         select
             *
           , `if`( course_id = '02', 1, 0 ) as sel_01_02
         from score
         -- join一定要写上 on, 否则产生灾难性后果, 数据量会爆炸
     ) t1
;
...
from (
         select
             *
           , sum( `if`( course_id = '01', 1, 0 ) ) as select_01 over()
             ,sum(`if`(course_id = '02', 1, 0)) select_02 over()
         from score s
     )
group by stu_id
;
...
where
  ...
  and exists
          -- 使用 exists的时候, 随便 select, *, 常数均可
          (
              select 1
              from score as sc_2
              where
                    sc_2.stu_id = score.stu_id
                and sc_2.course_id = '02'
          );
...
from (
         -- 全部学生所选课的集合
         select stu_id, collect_set( course_id ) as col_course_id
         from score s
         group by stu_id
     ) col_all
where
...
explain
    -- 没有显示所有成绩
select stu_id, avg( course ) as avg, sum( course ) as sum
from score s
group by stu_id
...
from (
         ...
         from (
                  select
                    ...
                    , course_id
                    , course
                    , avg( course ) over (partition by stu_id ) as avg_c
                  from score
              )           a_s
              join course c
              on a_s.course_id = c.course_id
     )            a_c
     join student s
     on a_c.stu_id = s.stu_id
...
from (
         ...
         from score s
         group by stu_id
         order by avg_course desc
     ) ini_data
;



select stu_id, avg( course ), rank( ) over (partition by stu_id) as rk
from score
group by stu_id
order by avg( course ) desc
;
...
from student
     left join
     (
         select stu_id, round( avg( course ), 2 ) as avg_score
         from score
         group by stu_id
     ) t1
...
-- 2022/11/3 19:52  join的位置代表其含义 , 不是所有的join都有意义,
-- 简单起见, 先join, 把所需要的数据先收集起来
...
from (
         select
           ...
           , stu.stu_name
           , sco.course
           , dense_rank( ) over (partition by course_id order by course desc) as rk
         from score        sco
              join student stu
...
from (
         select
           ...
           , t1.course_name
           , t1.course
           , dense_rank( ) over (partition by course_name order by course desc) as rk
         from (
                  select *
                  from student
                       left join score
...
select *
from (
         select s.course_id, s.stu_id, rank( ) over (partition by course_id order by course) as rk
         from score        s
              join student s2
              on s2.stu_id = s.stu_id
     ) t
...
from (
         select
           ...
           , t1.course
           , t1.tea_name
           , rank( ) over (partition by course_name order by course desc) as rk
         from (
                  select *
                  from student
...
select *, rank( ) over (partition by user_id order by active_date) as rk
from user_active ua
;

select user_id, sub_temp, count( active_date )
from (
         select user_id, date_sub( active_date, rk ) as sub_temp, *
         from (
                  select *, rank( ) over (partition by user_id order by active_date) as rk
                  from user_active ua
              ) tt
...
select *
from (
         select user_id, sub_temp, count( active_date ) as c
         from (
                  select user_id, date_sub( active_date, rk ) as sub_temp, *
                  from (
                           select *, rank( ) over (partition by user_id order by active_date) as rk
...
select *, rank( ) over (partition by category_id order by sum desc) as rk
from order_summary os
;
...
from order_summary os
     join category c
     on c.category_id = os.category_id
...
from (
         select
           ...
           , max( if( rk = 1, id, null ) ) as top_sale_product
         from (
                  select
                      category_id
                    , id
                    , rank( ) over (partition by category_id order by sum desc) as rk
                  from order_summary
                  where
                      test_case = 4 -- 测试用例数据集
              ) t1
...
select *, lag( buy_date, 1 ) over (partition by user_id order by buy_date) as lag
from (
         select user_id, buy_date
         from user_consum_details ucd
         group by user_id, buy_date
     ) t -- 去重
;

select datediff( lag_second_day, buy_date ) as date_sub
from (
         select *, lead( buy_date, 1 ) over (partition by user_id order by buy_date) as lag_second_day
         from (
                  select user_id, buy_date
                  from user_consum_details ucd
                  group by user_id, buy_date
        ...
        (
            ...
            from (
                     ...
                     from (
                              select
                                  *
                                , lead( buy_date, 1 ) over (partition by user_id order by buy_date) as lag_second_day
                              from (
                                       select user_id, buy_date
                                       from user_consum_details ucd
                                       group by user_id, buy_date
                                   ) t -- 2022/11/4 11:09 没有必要去重, 同一天没事
                          ) tt
                 ) ttt
            where
                date_sub = 1
...
from order_detail      od
     join product_attr pa
     on od.id = pa.id
where
      sale_date < date_sub( '2022-01-10', 30 )
...
select user_id, rank( ) over (partition by user_id order by active_date) as rk, active_date
...
select active_date, count( user_id ) as `新增用户数`
from (
         select *
         from (
                  select user_id, rank( ) over (partition by user_id order by active_date) as rk, active_date
...
from (
         ...
         from (
                  ...
                  where
                      active_date > date_sub( '2022-01-10', interval 90 day )
              ) t1
         where
             rk = 1
     ) t2
...
select *, rank( ) over (partition by id order by sum_id_date desc) as rk
from (
         select id, sale_date, sum( num ) as sum_id_date
         from order_detail2 o
         group by id, sale_date
     ) tt
;
...
select *
from (
         select *, rank( ) over (partition by id order by sum_id_date desc,sale_date ) as rk
         from (
                  select id, sale_date, sum( num ) as sum_id_date
                  from order_detail2 o
                  group by id, sale_date
...
from (
         select
           ...
           , row_number( ) over (partition by id order by sum( num ) desc ) as rk
         from order_detail2
         group by id, sale_date
     ) t1
where
...
from (
         select
           ...
           , o.num
           , o.sale_date
           , avg( num ) over (partition by category_id) as avg_num_cate
         from order_detail_3      o
              join product_attr_2 p
              on o.id = p.id
     ) tt
where
    num > avg_num_cate
...
select o.id, o.num, o.sale_date, avg( num ) over (partition by category_id) as avg_num_cate
from order_detail_3      o
     join product_attr_2 p
     on o.id = p.id
;
...
where
        num > (
                  select sum( num ) / count( id ) as avg_num
                  from order_detail_3 o
              )
order by id
;
...
from (
         select
           ...
           , name
           , num
           , avg( num ) over (partition by category_id) as cg_avg
         from (
                  select
                      id
                    , sum( num ) as num
...
from (
         select
           ...
           , max( if( register_rk = 1, active_date, null ) ) as register
         from (
                  select
                      user_id
                    , active_date
                    , row_number( ) over (partition by user_id order by active_date) as register_rk
                  from user_login_detail
              ) t1
         group by user_id
     )           t2
     left join (
                   select
                       user_id
                     , count( 1 ) as total_login_2021
                   from user_login_detail
                   where
                       year( active_date ) = 2021
                   group by user_id
               ) t3
     ...
     left join (
                   select
                       user_id
                     , count( 1 )   as order_num_2021
                     , sum( `num` ) as order_sum_2021
                   from order_detail_3
                   where
                       year( sale_date ) = 2021
                   group by user_id
...
from (
         select id
         from product_attr
     )           t2 -- 拿到所有产品,如果有最新修改价格,则为最新价格,没有则为默认99
     left join (
                   select
                       t1.id
                     , new_price
                   from (
                            select
                                id
                              , max( changeprice_date ) as new_date
                            from price_modification_details
                            where
                                changeprice_date <= '2022-04-20'
                            group by id
                        )                                    t1 -- 1. 找到指定日期前,商品的最新的修改日期
...
where
        user_id in
        (
            select if( user1_id = 1, user2_id, user1_id ) as uid
            from friendship
            where
                user1_id = 1
              or user2_id = 1
...
from (
         select
             user_id
           , active_date
           , date_sub( active_date, rk ) as flag
         from (
                  select
                      user_id
                    , active_date
...
select
    sale_date
  , sum( if( sex = '男', price, 0 ) ) as male
  , sum( if( sex = '女', price, 0 ) ) as female
...
from (
         select
             sale_date
           , sum( today_total ) over (order by sale_date rows between 6 preceding and current row ) as total_7
           , rk
         from (
                  select
                      sale_date
                    , row_number( ) over (order by sale_date) as rk -- 过滤走不是一个完整的七天的数据
                    , sum( price )                            as today_total
                  from order_detail_5
                  group by sale_date
              ) t1 -- 计算出每日销售总额
     ) t2
...
from (
         select
             id
           , user_id
           , if( id = 1, 1, 0 )  as flag1
           , if( id = 2, 1, 0 )  as flag2
           , if( id = 3, -5, 0 ) as flag3
         from order_detail_6
     ) t1
group by user_id
having
    sum( flag1 + flag2 + flag3 ) = 2
;
结果
：
user_id
...
from (
         select
             sale_date
           , sum( if( id = 1, num, 0 ) ) as f1
           , sum( if( id = 2, num, 0 ) ) as f2
         from order_detail_7
         group by sale_date
     ) t1
;
...
from (
         select
           ...
           , order_id
           , sale_date
           , row_number( ) over (partition by user_id order by sale_date desc) as rk
         from order_detail_8
     ) t1
where
    rk <= 3
...
INTO
    user_active_4
VALUES
    ( '1', '1', '2021-02-01 01:00:00', '2021-02-01 01:30:00' )
...
from (
         select
             user_id
           , ip_address
           , unix_timestamp( login_ts )                                                             as `in`
           , unix_timestamp( logout_ts )                                                            as `out`
           , lead( unix_timestamp( login_ts ), 1 ) over (partition by user_id order by ip_address)  as next_in
           , lead( unix_timestamp( logout_ts ), 1 ) over (partition by user_id order by ip_address) as next_out
         from (
                  select
                      user_id
                    , ip_address
                    , max( login_ts )  as login_ts
                    , max( logout_ts ) as logout_ts
                  from user_active_4
                  group by user_id, ip_address
              ) t2
     ) t1
where
     ( next_in < `in` and next_out >= `in` )
...
from (
         ...
         from (
                  select
                      id
                    , date_format( sale_date, '%y-%m' ) as dt
                    , max( ( sale_date ) )              as sale_date
                    , sum( price )                      as total_mth
                  from order_info
                  group by date_format( sale_date, '%y-%m' ), id
              )                        t1
              left join product_supply t2
              on t1.id = t2.id
         where
             total_mth > assignment
     ) t3
...
年6
月
，销售额为 107100 + 102100 + 90900 = 300100
。
在 2021
年7
月
...
from (
         ...
         from (
                  select
                      t1.user_id
                    , if( action = 'success', 1, 0 ) as suc
                    , if( action = 'timeout', 1, 0 ) as fail
                  from user_info_2              t1
                       left join paoment_detail pd
                       on t1.user_id = pd.user_id
              ) t1
         group by user_id
     ) t2
;
...
from (
         select
             id
           , date
           , sum( if( action = 'sell', -amount, amount ) ) as remain
         from stock_detail
         group by id, date
     ) t1
;
...
from (
         select
           ...
           , c.category_id
           , sum
           , rank( ) over (partition by c.category_id order by sum desc) as rk
           , category_name
         from order_summary_1
              left join category c
              on order_summary_1.category_id = c.category_id
     ) t1
where
    rk <= 3
;
...
from (
         ...
         from (
                  select
                    ...
                    , count( category_id ) over (partition by category_id)         as cnt
                  from produt_detail
              ) t1
         where
             rk in ( floor( ( cnt + 1 ) / 2 ), floor( ( cnt + 2 ) / 2 ) ) -- 偶数中位数原样输出
...
from (
         select
             *
           , count( * ) over (partition by flag) as cnt
         from (
                  select
                      order_id
                    , date
                    ...
                    , date_sub( date
                          , row_number( ) over (order by date) ) as flag
                  from order_detail_9
                  where
                          price
                          > 100
...
select
    register_date
  , register
  , remain_1 / register as retention
from (
         select
             register_date
           , count( a.user_id ) as register -- 新增用户
           , count( b.user_id ) as remain_1 -- 次日留存
         from (
                  select
                      user_id
...
from (
         select
             status
           , date
           , row_number( ) over (partition by status order by date)         as rk
           , date_sub( date
                 , row_number( ) over (partition by status order by date) ) as diff
         from (
                  select *
                  from (
                           select
...
代码实现
select
    nvl( sale_count, rnb ) as sale_count
  , nvl( login_count, 0 )  as login_count
from (
         SELECT 0 AS rnb
         UNION
         SELECT ROW_NUMBER( ) OVER () AS rnb
         FROM translation_recode
     ) t3 -- 第二步:通过交易表生成序列号,并且加上0号序列,主要是解决 结果表1中没有sale_count为2的问题
     left join
     (
         select
             sale_count
           , count( sale_count ) as login_count
         from (
                  select
                      register.user_id
                    , register.date
                    , nvl( sale_count, 0 ) as sale_count-- 将null转为0方便统计
                  from register
                       left join (
                                     select
                                         user_id
                       ...
                       on register.user_id = t1.user_id and register.date = t1.date
              ) t2
...
where
        rnb <= (
                   ...
                   limit 1
               ) -- 第三步: 与t4表内容大致一样 目的是限制序列号,也就是登录一次,交易五次\六次\七次的记录不要 因为大部分都为0
;
结果
：
...
select
    b.category
  , nvl( sum( case when dayofweek( a.order_date ) = 2 then a.num end ), 0 ) as Monday
  , nvl( sum( case when dayofweek( a.order_date ) = 3 then a.num end ), 0 ) as Tuesday
  , nvl( sum( case when dayofweek( a.order_date ) = 4 then a.num end ), 0 ) as Wednesday
  , nvl( sum( case when dayofweek( a.order_date ) = 5 then a.num end ), 0 ) as Thursday
  , nvl( sum( case when dayofweek( a.order_date ) = 6 then a.num end ), 0 ) as Friday
  , nvl( sum( case when dayofweek( a.order_date ) = 7 then a.num end ), 0 ) as Saturday
  , nvl( sum( case when dayofweek( a.order_date ) = 1 then a.num end ), 0 ) as Sunday
...
from (
         select
           ...
           , last_value( price )
                         over
                             (partition by product_id order by start_date rows between unbounded preceding and unbounded following)
                 - first_value( price )
                                over (partition by product_id order by start_date rows between unbounded preceding and unbounded following) as growth
           , lag( end_date, 1, '9999-01-01' )
                  over (partition by product_id order by start_date)                                                                        as `date`
           , row_number( ) over (partition by product_id order by end_date desc )                                                           as rk
         from price_change
     ) t1
where
    rk = 1
...
36.1
题目需求
分析如果有一个用户成功下单两个及两个以上的购买成功的手机订单
（购买商品为xiaomi
，apple
，vivo
）那么输出这个用户的id
及第一次成功购买手机的日期和第二次成功购买手机的日期
，以及购买手机成功的次数
。
36.2
表结构
...
INTO
    order_info_2
VALUES
    ( '1', '100001', 'xiaomi', 'fail', '2021-01-01' )
...
from (
         select
           ...
           , lag( date, 1 ) over (partition by user_id order by date) as first
           , row_number( ) over (partition by user_id order by date)  as rk
         from order_info_2
         where
               product_name in ( 'xiaomi', 'apple', 'vivo' )
           and status = 'success'
     ) t1
where
      cnt >= 2
...
from (
         select
           ...
           , sum( num )                   as total
           , max( month( date ) )         as mn
         from product_detail_3
         where
             year( date ) = '2021'
         group by product_name, date_format( date, '%Y-%m' )
     )      t1
     join (
              select
                  product_name
...
from (
         select
             id
           , agg
           , rank( ) over (order by agg desc) as rk
         from (
                  select
                      id
                    , sum( agg ) as agg
                  from (
                           select
                               id
                             , if( action = 'supply', amount, -amount ) as agg
                           from product_detail
                       ) t1
                  group by id
              ) t2
...
with favor          as ( -- 初步获取收藏次数
                           select
                             ...
                             , favor_cnt
                             , sum( favor_cnt )
                                    over (partition by category order by favor_date range between interval '6' day preceding and current row ) as favor_total
                           
                           from (
                                    select
                                        category
                                      , favor_date
                                      , count( 1 ) as favor_cnt
                                    from (
                                             select
                                                 date as favor_date
                                               , category
   ...
   , t1             as ( -- 第一步
                           select
                               dic.category
                             , dt
                             , nvl( favor_total, 0 ) as favor_total
   ...
   , favor_final    as ( -- 将前三天的日期补充进来后重算total,此步可以优化
                           select
                             ...
                             , favor_total
                             , sum( favor_cnt )
                                    over (partition by category order by str_to_date( favor_date, '%Y-%m-%d' ) range between interval '6' day preceding and current row ) as favor_final
   ...
   , purchase       as (
                           select
                             ...
                             , puchase_cnt
                             , sum( puchase_cnt )
                                    over (partition by category order by purchase_date range between interval '6' day preceding and current row ) as puchase_total
                           from (
                                    select
                                        category
                                      , purchase_date
                                      , count( 1 ) as puchase_cnt
                                    from (
                                             select
   ...
   , t2             as (
                           select
                               dic.category
                             , dt
                             , nvl( puchase_total, 0 ) as puchase_total
                             , nvl( puchase_cnt, 0 )   as puchase_cnt
                           from dic
                                left join purchase
   ...
   , purchase_res   as (
                           select category, dt as purchase_date, puchase_cnt, puchase_total
                           from t2
                           union
   ...
   , purchase_final as (
                           select
                             ...
                             , puchase_total
                             , sum( puchase_cnt )
                                    over (partition by category order by str_to_date( purchase_date, '%Y-%m-%d' ) range between interval '6' day preceding and current row ) as puchase_final
                               -- order字段必须是date类型
                           from purchase_res
                       )
select
...
where
    purchase_date between '2021-10-01' and '2021-10-03'
;
结果
：
...
INTO
    user_action_log
VALUES
    ( '101', '9001', '2021-11-01 02:00:00', '2021-11-01 02:00:11' )
...
from (
         select
             id
           , sum( flag ) over (partition by id order by `time`) as cnt
         from (
                  select
                      id
                    , start_time as `time`
...
INTO
    user_action_log_2
VALUES
    ( '109', ' 9001', '2021-08-31 02:00:00', '2021-08-31 02:00:09' )
...
INTO
    user_sign
VALUES
    ( '101', '0', '2021-07-07 02:00:00', '2021-07-07 02:00:09', 1 )
...
from (
         ...
         from (
                  select
                      user_id
                    , login
                    , row_number( ) over (partition by user_id,flag order by login) as ctn
                  from (
                           select
                               user_id
                             , date_format( start_time, '%Y-%m-%d' )                                                                 as login
                             , row_number( )
                                   over
                                       (partition by user_id order by date_format( start_time, '%Y-%m-%d' ) )                        as rk
                             , date_sub( date_format( start_time, '%Y-%m-%d' ),
                                         row_number( ) over (partition by user_id order by date_format( start_time, '%Y-%m-%d' ) ) ) as flag
                           from user_sign
                           where
                               sign = 1
                       ) t1
...
INTO
    user_action_2
VALUES
    ( '101', ' 8001', '2021-10-01 02:00:00', 0, 0, 0, 0 )
...
select
    id
  , round( cli_cnt / show_cnt, 3 ) as click
  , round( cart_cnt / cli_cnt, 3 ) as cart
  , round( pay_cnt / cart_cnt, 3 ) as pay
  , round( re_cnt / pay_cnt, 3 )   as refund
from (
         select
             id
           , sum( if( refund = 1, 1, 0 ) )  as re_cnt
           , sum( if( payment = 1, 1, 0 ) ) as pay_cnt
           
           , sum( if( click = 1, 1, 0 ) )   as cli_cnt
           , count( 1 )                     as show_cnt
           , sum( if( cart = 1, 1, 0 ) )    as cart_cnt
...
代码实现
select
    round( sum( total_amount ) / count( 1 ), 1 )
...
from (
         select
           ...
           , user_id
           , total_amount
           , row_number( ) over (partition by user_id order by event_time ) as rk
           , sum( price * `count` ) over (partition by b.order_id)          as product_total
...
45.1
题目需求
动销率定义为店铺中一段时间内有销量的商品占当前已上架总商品数的比例
（有销量的商品/
已上架总商品数)
。
滞销率定义为店铺中一段时间内没有销量的商品占当前已上架总商品数的比例
。（没有销量的商品/
已上架总商品数)
。
